...
parser.add_argument('--kernel', action="store", dest="kernel", type=str, default='Laplacian',
...
def main():
    ...
    fVU: str = "%s_VU[%05d,%.5f,%.5f,%05d]" % (FLAGS.kernel, FLAGS.eta, FLAGS.epsilon, FLAGS.scale, FLAGS.rank)
    if FLAGS.reuse and path.exists(fVU):
        logging.info(f"Step 2. restoring model from {fVU}")
