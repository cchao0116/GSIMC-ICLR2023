...
class Model:
    ...
    def kernel(self, tr_data: sp.csr_matrix, kernel: Kernel):
        ...
        fKernel: str = "%s[%05d,%05d,%.5f,%.5f]" % (FLAGS.kernel, FLAGS.rank, FLAGS.eta, FLAGS.epsilon, FLAGS.scale)
        if FLAGS.reuse and path.exists(fKernel):
            logging.info(f"Step 2. ...load kernel matrix from {fKernel}")
    ...
    def evaluate(self, test_tr: sp.csr_matrix, test_te: sp.csr_matrix,
        ...
        HR = np.mean(np.sign(Pu))  # Hit-Rate@100

        Pu_50 = np.sum(res[:, :50], 1).astype(np.float32)
        HR_50 = np.mean(np.sign(Pu_50))  # Hit-Rate@50
        ...
        NDCG = np.mean(DCG / pDCG)  # NDCG@100
        logging.info("=====HR@50: {0:.5f}, HR@100: {1:.5f}, NDCG@100: {2:.5f}====="
                     .format(HR_50, HR, NDCG))
